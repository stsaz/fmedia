...
static void this_meta_fill(JNIEnv *env, jobject thiz, fmed_que_entry *qe);
...
Java_com_github_stsaz_fmedia_Fmedia_meta(JNIEnv *env, jobject thiz, jint list_item, jstring jfilepath)
{
	dbglog0("%s: enter", __func__);
	fftime t1 = fftime_monotonic();
	fmed_track_obj *t = fx->track->create(FMED_TRK_TYPE_METAINFO, NULL);
	fmed_track_info *ti = fx->track->conf(t);
	jclass jc = jni_class_obj(thiz);
	ffvec info = {};

	ti->que_cur = (void*)fx->qu->cmdv(FMED_QUE_ITEM, (int)-1, (ffsize)list_item);
	FF_ASSERT(ti->que_cur != NULL);
	if (ti->que_cur == NULL)
		goto end;
	...
	ti->que_cur->dur = msec;
	this_meta_fill(env, thiz, ti->que_cur);
...
static void this_meta_fill(JNIEnv *env, jobject thiz, fmed_que_entry *qe)
{
	...
	char *artist, *title;
	ffstr *val;
	if (NULL != (val = fx->qu->meta_find(qe, FFSTR("artist"))))
		artist = ffsz_dupstr(val);
	else
		artist = ffsz_dup("");

	if (NULL != (val = fx->qu->meta_find(qe, FFSTR("title"))))
	...
	jni_obj_sz_set(env, thiz, jf, qe->url.ptr);
	...
	jni_obj_long_set(thiz, jf, qe->dur);
...
Java_com_github_stsaz_fmedia_Fmedia_quCmd(JNIEnv *env, jobject thiz, jlong q, jint cmd, jint i)
{
	...
	case QUCOM_META: {
		fmed_que_entry *qe = (void*)fx->qu->cmdv(FMED_QUE_ITEM, qi, (ffsize)i);
		this_meta_fill(env, thiz, qe);
		break;
	}
	}
	return 0;
